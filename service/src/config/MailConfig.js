...
async function send (sendInfo) {
  ...
  const url = `${BASE_URL}/#${route}?${qs.stringify(sendInfo.data)}`
  ...
  const info = await transporter.sendMail({
    ...
    subject:
      sendInfo.user !== '' && sendInfo.type !== 'email'
        ? `你好开发者，${sendInfo.user}！《熊猫社区重置密码》验证邮件`
        : `你好开发者，${sendInfo.user}！《熊猫社区重置登录邮箱》验证邮件`, // Subject line
    ...
    html: `
      <div style="border: 1px solid #dcdcdc;color: #676767;width: 600px; margin: 0 auto;    padding-bottom: 50px;position: relative;">
        ...
        <div style="padding: 25px">
          <div>您好，${sendInfo.user}童鞋，重置链接有效时间30分钟，请在${sendInfo.expire}之前点击重置按钮进行下一步操作：</div>
          <a href="${url}" style="padding: 10px 20px; color: #fff; background: #009e94; display: inline-block;margin: 15px 0;">立即重置</a>
